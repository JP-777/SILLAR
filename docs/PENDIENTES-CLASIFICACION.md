...
| **3** | Tres copias de `StampReplicationColumns` | «La cuarta copia, **o** la primera vez que dos copias discrepen» | **NO cumplido, y comprobado:** siguen siendo **tres** (`CoreDbContext.cs:133`, `CatalogDbContext.cs:92`, `CrmDbContext.cs:86`) y los tres cuerpos son **idénticos** salvo el nombre del campo inyectado (`clock` / `_clock`). Ninguna de las dos mitades del disparador se ha cumplido |
...
Eso cambia la entrada de sitio, y por eso no se toca aquí: parte de lo que la 8 pedía —«que
un fallo por entorno se distinga de uno de código sin depender de que alguien lea el log con
criterio»— ahora tiene al menos un manual de reconocimiento, pero **sigue dependiendo de que
alguien lo lea**. La 8 pedía que el arnés lo dijera por su nombre, y eso no está hecho. Si se
cierra, se cierra sabiendo qué mitad queda fuera.
...
| **13** | Arranque con base vacía revienta con `42P01` | «La primera pantalla que vería quien instale en una clienta» — pero no dice cuándo |
