...
Confirmado el 20 de agosto de 2026 sobre el stack exclusivo `sillar_m02`, después de eliminar `sillar_m02_db_data` y aplicar CORE, Catálogo y CMS en un clúster vacío:

- La migración `20260820050000_CmsInitial` creó el schema `cms`, su historial y las cinco tablas.
- PostgreSQL informó `uuid` y `NULL` permitido para las cinco columnas de imagen, además de `alt_text` nullable en banners, promociones y trabajos destacados.
- Las cinco FK hacia `core.media_assets` mostraron `ON DELETE SET NULL` (`confdeltype = n`); no quedó ninguna FK de medios con `RESTRICT`.
- Un banner, una promoción y un trabajo sin imagen ni `alt_text` se insertaron correctamente dentro de una transacción.
- Con una imagen real, los tres intentos sin `alt_text` fallaron respectivamente contra `ck_banners_alt_text_si_hay_imagen`, `ck_promotions_alt_text_si_hay_imagen` y `ck_featured_projects_alt_text_si_hay_imagen`.
- Borrar de CORE un medio usado por un banner terminó correctamente y dejó `image_desktop_id` en `NULL`, conservando la fila y su texto alternativo.
- Las pruebas se revirtieron; las cinco tablas quedaron con cero filas de negocio y ninguna contiene `origin_node` ni `row_version`.
- EF Core no encontró cambios pendientes entre el modelo y el snapshot. La solución compiló con 0 advertencias y 0 errores; pasaron 187 pruebas (132 CORE, 49 Shared y 6 CMS).
